...
def collect_participants_or_raise(
    ...
    for pair in couple_rows:
        ...
        if not EMAIL_RE.match(a_email) or not EMAIL_RE.match(b_email):
            raise ValueError("Gebruik geldige e-mailadressen voor elk koppel.")
    ...
    for s_name_e, s_email_e in single_rows:
        ...
        if not EMAIL_RE.match(s_email):
            raise ValueError(f"Gebruik een geldig e-mailadres voor single “{s_name or '—'}”.")
